...
{
  LOAD_FIRST_SCENE,
  FIRST_SCENE_FIRST_CAPTURE,
  FIRST_SCENE_SECOND_CAPTURE,
  LOAD_SECOND_SCENE,
  SECOND_SCENE_FIRST_CAPTURE,
...
private:
  ...
  void PerformTest()
  {
    ...
    switch (gTestStep)
    {
      ...
      case FIRST_SCENE_FIRST_CAPTURE:
      {
        if (mAnimation)
        {
          mAnimation.Play();
      ...
      case SECOND_SCENE_FIRST_CAPTURE:
      {
        if (mAnimation)
        {
          mAnimation.Play();
  ...
  void PostRender()
  {
    ...
    if (mAnimation)
    {
      delay = mAnimation.GetDuration() * 1000 + 1000;
    }

    if (gTestStep == FIRST_SCENE_FIRST_CAPTURE)
    {
    ...
    else if ( gTestStep == SECOND_SCENE_FIRST_CAPTURE )
    {
      DALI_ASSERT_ALWAYS(CheckImage(THIRD_IMAGE_FILE)); // should be identical

      WaitForNextTest(delay);
    }
    else if ( gTestStep == SECOND_SCENE_SECOND_CAPTURE )
    {
      DALI_ASSERT_ALWAYS(CheckImage(FOURTH_IMAGE_FILE)); // should be identical

      WaitForNextTest(1000);
    }
    else if ( gTestStep == THIRD_SCENE_CAPTURE )
    {
      DALI_ASSERT_ALWAYS(CheckImage(FIFTH_IMAGE_FILE)); // should be identical
